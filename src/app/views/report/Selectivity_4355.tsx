...
const Selectivity_4355 = () => {
  ...
  const handleSubmit = async (data: SearchData) => {
    try {
      setLoading(true)
      const res = await axios.post('/reporting/SelectivityReport4355', {
  ...
  return (
    <SimpleCard title={t(`${translationsForReportSelectivity4355}.title`)}>
      ...
      <Box width="100%" overflow="auto">
        <DataTable
          exportFilename={`Selectivity Report 4355 ${new Date().toISOString()}`}
          ref={tableRef}
          value={reportData}
          rows={ROWS_PER_PAGE}
...
export default Selectivity_4355;
